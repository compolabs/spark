    ...
    return <Root>
        <Row alignItems="center">
            ...
            {menuItems.map(({title, link, route}, key) => {
                if (link == null && route == null) return <MenuItem key={key} disabled>{title}</MenuItem>
                else if (route != null) return <Link to={route}>
                    <MenuItem active={isRoutesEquals(route, location.pathname)} key={key}>{title}</MenuItem>
                </Link>
                else if (link != null) return <a rel="noopener noreferrer" target="_blank" href={link}>
        ...
        <Row alignItems="center" justifyContent="flex-end">
            <SizedBox width={10}/>
            <SettingsButton outline><GearIcon/></SettingsButton>
            <SizedBox width={10}/>
            <Button style={{height: 32}} outline fitContent>DEPOSIT/WITHDRAW</Button>
            <SizedBox width={10}/>
            <Button style={{height: 32}} outline fitContent>BRIDGE</Button>
            <SizedBox width={10}/>
            {accountStore.address != null
                ? <Button fitContent
                          onClick={accountStore.disconnect}>Disconnect {centerEllipsis(accountStore.address, 8)}</Button>
                : <Button fitContent onClick={() => accountStore.login(LOGIN_TYPE.FUEL_WALLET)}>Connect wallet</Button>
            }
        </Row>
    </Root>;
})
